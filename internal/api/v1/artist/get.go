...
func getArtistHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		httputils.SendErrorToClient(w, httputils.NewBadRequestError("Invalid artist ID"))
		return
	} else if artist_id < 0 {
		httputils.SendErrorToClient(w, httputils.NewBadRequestError(fmt.Sprintf("Invalid artist ID: %d", artist_id)))
		return
	}
...
func getArtistsHandler(w http.ResponseWriter, r *http.Request) {
	...
	if params["artist_id"] != nil && len(params) > 1 {
		httputils.SendErrorToClient(w, httputils.NewBadRequestError("Can't use artist_id with other parameters"))
		return
